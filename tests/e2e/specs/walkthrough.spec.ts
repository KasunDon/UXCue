...
const DRAFT = {
  element: {
    ...
    outerHtmlSkeleton:
      '<button data-testid="upgrade-plan-button" class="btn btn-primary"></button>',
    bbox: {
...
test("visual walkthrough of the review loop", async ({ context, extensionId }) => {
  ...
  page.on("dialog", (d) =>
    d.accept(d.message().includes("session") ? "Billing polish" : "KtKAI Console"),
  );
  ...
  await page
    .getByTestId("composer-feedback")
    .fill("The primary button label wraps onto two lines and misaligns with Cancel.");
    ...
    (d) =>
      chrome.storage.local.set({
        captureDraft: {
          ...
          element: {
            ...d.element,
            selector: "nav.app-nav > a:nth-of-type(2)",
            textSnippet: "Billing",
          },
          page: { ...d.page, pathname: "/responsive" },
        },
      }),
    DRAFT,
  );
  ...
  await page
    .getByTestId("composer-feedback")
    .fill("Below 640px the nav is absolutely positioned and overlaps the page title.");
